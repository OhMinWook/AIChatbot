...
interface BotChatErrorUIProps {
  errorMessage: string;
  date: number;
}

export default function BotChatErrorUI({
  errorMessage,
  date,
}: BotChatErrorUIProps) {
  return (
      ...
      <article className='w-full flex space-x-3'>
        ...
        <section className='w-full flex flex-col space-y-5 md:space-y-8'>
          ...
          <div className='w-full text-2xl'>
            <article className='flex text-red-500 justify-center items-center md:items-start bg-customColor-white md:bg-transparent border-1 border-customColor-bg3 rounded-2xl w-fit px-8 md:px-0 py-3 md:py-0 h-auto min-h-16'>
              {errorMessage}
            </article>
          </div>
        </section>

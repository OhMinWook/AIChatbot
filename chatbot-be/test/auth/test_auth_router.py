...
async def test_sign_in(
    ...
    # given
    sign_in_router, _ = auth_router
    sign_in_service, _ = auth_service
    mock_request.session = {config.COOKIE_NAME: "mocked_session_id"}
    ...
    async def mock_sign_in_router(sign_in_request, db_session):
        ...
        response.set_cookie(
            key=config.COOKIE_NAME, value=mock_request.session[config.COOKIE_NAME], httponly=True,
    ...
    assert any(config.COOKIE_NAME in cookie for cookie in cookies)
    deleted_cookies = [cookie for cookie in cookies if f'{config.ADMIN_COOKIE_NAME}=' in cookie]
...
async def test_sign_out(
    ...
    # given
    _, sign_out_router = auth_router
    _, sign_out_service = auth_service

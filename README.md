...
| 단계                        | 설명                                                                                              |
|----------------------------|--------------------------------------------------------------------------------------------------|
| **PDF 데이터 추출 (Text 및 이미지)** | PDF 파일을 `fitz` 라이브러리를 사용하여 열고, 각 페이지에서 텍스트와 이미지를 추출합니다. 텍스트는 특정 영역을 지정하여 페이지에서 추출하고, 이미지는 PDF 페이지를 이미지로 변환하여 추출합니다. |
| **패턴 매칭 및 데이터 구조화**   | 각 페이지의 텍스트는 사전 정의된 패턴에 맞춰 분류됩니다. 패턴은 `pattern1`, `pattern1_1`, `pattern2` 등으로 정의되며, 이를 통해 페이지의 구조에 맞는 데이터만 추출됩니다. `manual_pattern_changer` 함수는 패턴을 변경하여 각 페이지의 텍스트와 이미지가 적절히 처리될 수 있도록 합니다. |
| **OpenAI API 호출을 통한 데이터 처리** | 각 페이지의 텍스트와 이미지 데이터를 OpenAI의 GPT 모델에 맞춰 전송하여, 특정 형식으로 응답을 받아옵니다. 응답에는 `source`, `subject`, `content`가 포함되며, 후속 작업에 사용됩니다. `get_completion_from_messages` 함수는 OpenAI API를 호출하여 처리된 데이터를 받아옵니다. |
| **데이터 처리 결과 저장**      | 각 페이지의 결과는 `self.data_dict`에 저장되며, 텍스트와 이미지, 패턴에 대한 정보가 포함됩니다. 이 데이터는 후속 처리에 활용됩니다. |
| **토큰 계산 및 관리**         | 각 API 호출 시 사용된 토큰 수를 추적하여 `self.tokens_dict`에 저장합니다. 이를 통해 API 호출 비용을 관리하고, 필요한 경우 다시 시도할 수 있는 로직을 추가합니다. |
| **최종 결과 반환**            | 모든 페이지에 대한 전처리가 완료되면, 최종적으로 구조화된 데이터와 토큰 사용 내역을 반환합니다. 이를 통해 시스템은 PDF 문서의 데이터를 정확하게 처리하고, 필요한 형태로 변환할 수 있습니다. |
